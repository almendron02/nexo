...
export function CourseModuleOverview({ module }: { module: CourseCatalogModule }) {
      ...
      <section className="lesson-list-section">
        ...
        <div className="lesson-list">
          {module.lessons.map((lesson, index) => {
            ...
            const available = module.available && (completed || index === 0 || state.completedLessons.includes(module.lessons[index - 1].id));
            ...
            return (
              <article className={`lesson-row ${isNext ? "is-next" : ""}`} id={`lesson-${lesson.id}`} key={lesson.id}>
                ...
                <div className="lesson-row__action">
                  ...
                  {available ? <Link className="round-link" href={`/lesson/${lesson.id}`} aria-label={`${completed ? "Revisit" : "Open"} ${lesson.title}`}><ArrowRight aria-hidden="true" /></Link> : <span className="locked-label">{module.available ? "Complete the prior lesson" : "Planned"}</span>}
                </div>
              </article>
            );
          })}
