...
export function CheckpointGate({ access, stage }: { access: Exclude<LessonAccess, { status: "available" }>; stage: number }) {
  ...
  return (
    <main className="access-page">
      ...
      <h1>{account ? "Create an account to continue." : "Finish the stage before its checkpoint."}</h1>
      <p>{account ? "The complete course is free. Your account keeps checkpoint results and the course path together." : `The checkpoint unlocks when every preceding lesson is complete. Your next required class is Lesson ${access.status === "prerequisite-required" ? access.prerequisiteId : ""}.`}</p>
      <div className="access-page__actions">
        {account ? <Link className="button button--dark" href={`/auth/sign-up?next=${encodeURIComponent(next)}`}>Create account <ArrowRight aria-hidden="true" /></Link> : access.status === "prerequisite-required" ? <Link className="button button--dark" href={`/lesson/${access.prerequisiteId}`}>Continue Lesson {access.prerequisiteId} <ArrowRight aria-hidden="true" /></Link> : null}
